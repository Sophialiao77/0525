...
function setup() {
  ...
  createCanvas(windowWidth, windowHeight);  
  for(var i=0;i<100 ;i=i+1){  //i=0，1，2，3，4，.......，8，9
    monster = new Monster({})  //產生一個新的Monster class元件
    monster.push(monster)  //把ball的物件放入到balls陣列內
  }
}
function draw() {
  background(220);
  ...
  if(keyIsPressed){
    ...
    if(key=="ArrowRight"|| key=="d"){ //按下鍵盤的往右鍵d鍵
      shipP.x = shipP.x +5
    }
    if(key=="ArrowUp"|| key=="w"){  //按下鍵盤的往上鍵w鍵
      shipP.y = shipP.y -5
    }
    
    if(key=="ArrowDown"|| key=="s"){  //按下鍵盤的往下鍵s鍵
      shipP.y = shipP.y +5
    }
  }
  ...
  //怪物的顯示
  for(let monster of monster)  //只要是陣列的方式，都可以利用此方式處理
  {
    if monster.dead == true && monster.timenum
    monster.draw()
    monster.update()
  
    for(let bullet of bullets){   //檢查每一個飛彈物件
      if(ball.isBallInRanger(bullet.p.x,bullet.p.y)){    //飛彈物件有沒有接觸現在的ball
        monster.splice(balls.indexOf(monster),1)  //從倉庫balls取出被滑鼠按到的物件編號(balls.indexOf(ball))，只取1個
        bullets.splice(bullets.indexOf(bullet),1)
        score = score + 1
        //elephant_sound.play()
      }
  ...
  push()  //重新規劃原點(0,0)，在視窗的中間
    ...
    translate(shipP.x,shipP.y)   //把砲台的中心點放在視窗的位置
    fill("#b5179e")
    noStroke()
    rotate(angle)
